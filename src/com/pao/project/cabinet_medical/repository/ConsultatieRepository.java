...
public class ConsultatieRepository implements Repository<Consultatie, Integer> {
    ...
    public List<String> findConsultatiiPerClient(int clientId) {
        ...
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ...
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    rezultate.add(String.format(
                            "Client: %s | Programare: %s | Diagnostic: %s | " +
                                    "Cost: %.2f | Data: %s",
                            rs.getString("client"),
                            rs.getString("cod_programare"),
                            rs.getString("diagnostic"),
                            rs.getDouble("cost_final"),
                            rs.getTimestamp("data_consultatie").toLocalDateTime()
                    ));
                }
            }
        } catch (SQLException e) {
    ...
    private Consultatie mapRow(ResultSet rs) throws SQLException {
        ...
        Programare programareMin = new Programare(
                null, null, null, 0,
                new com.pao.project.cabinet_medical.model.CodProgramare(
                        rs.getString("cod_programare")
                )
        );
